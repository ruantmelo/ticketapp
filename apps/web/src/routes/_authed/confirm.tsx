...
function StatusAlert({ event }: { event: EventPublished }) {
  if (event.status === "mint_failed") {
    return <Alert variant="error" title="Evento criado, mas o minting falhou" className="w-full max-w-[560px]">{event.mintError ?? "Confira o painel para tentar o retry do minting."}</Alert>;
  }
  if (event.status === "minting") {
    return <Alert variant="warning" title="Evento criado. Minting em andamento" className="w-full max-w-[560px]">Seus ingressos estão sendo mintados na blockchain. Você acompanhará o progresso no painel.</Alert>;
  }
  return <Alert variant="success" title="Evento mintado com sucesso!" className="w-full max-w-[560px]">Os ingressos foram criados e estão prontos para os próximos passos no painel.</Alert>;
}
